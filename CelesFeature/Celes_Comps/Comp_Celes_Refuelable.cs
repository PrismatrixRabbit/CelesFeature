...
namespace CelesFeature
{
    ...
    public class Comp_Celes_Refuelable : ThingComp_VacuumAware, IThingGlower
    {
        private float fuel;

        private bool isIdle = false;

        public float extraFuelConsumption = 0f;
        ...
        public Comp_Celes_Linker Linker;

        public Comp_Celes_ConsumptionFactors ConsumptionFactors;
        ...
        public CompProperties_Celes_Refuelable Props => (CompProperties_Celes_Refuelable)props;
        ...
        private float ConsumptionRatePerTick
        {
            get
            {
                float num = Props.fuelConsumptionRate + extraFuelConsumption;
                if (this.isIdle)
                {
                    if (ConsumptionFactors != null)
                    {
                        num *= ConsumptionFactors.Props.fuelIdleFactor;
                    }
                }
                return num / 60000f;
            }
        ...
        public override void PostSpawnSetup(bool respawningAfterLoad)
        {
            ...
            ConsumptionFactors = parent.TryGetComp<Comp_Celes_ConsumptionFactors>();
        }

        public override void PostExposeData()
        {
            base.PostExposeData();
            Scribe_Values.Look(ref isIdle, "Celes_SaveKey_bool_Comp_Refuelable_isIdle");
            Scribe_Values.Look(ref fuel, "Celes_SaveKey_float_Comp_Refuelable_fuel", 0f);
            Scribe_Values.Look(ref extraFuelConsumption, "Celes_SaveKey_float_Comp_Refuelable_extraFuelConsumption");
            Scribe_Values.Look(ref configuredTargetFuelLevel, "Celes_SaveKey_float_Comp_Refuelable_configuredTargetFuelLevel", -1f);
            Scribe_Values.Look(ref allowAutoRefuel, "Celes_SaveKey_bool_Comp_Refuelable_allowAutoRefuel", defaultValue: false);
            Scribe_Values.Look(ref tickRare, "Celes_SaveKey_float_Comp_Refuelable_tickRare");
        ...
        public override void PostDestroy(DestroyMode mode, Map previousMap)
        {
            ...
            if (mode != 0 && previousMap != null && Props.fuelFilter.AllowedDefCount == 1 && Props.initialFuelPercent == 0f)
            {
        ...
        {
            this.isIdle = parent.IsIdle();
            this.extraFuelConsumption = 0;
            if (parent.GetType() == typeof(Building_Celes_Core))
            {
                if (Linker != null && Linker.linkedThings.Count > 0)
                {
                    foreach (Thing thing in Linker.linkedThings)
                    {
                        ...
                        if (comp != null)
                        {
                            if (comp.Props.extraFuelConsumptionForCore > 0)
                            {
                                this.extraFuelConsumption += comp.Props.extraFuelConsumptionForCore;
                            }
                        }
                    }
                }
        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            if (!Props.hideGizmosIfNotPlayerFaction || parent.Faction == Faction.OfPlayer)
            {
                ...
                {
                    yield return new Gizmo_Celes_SetFuelLevel(this);
                }
                else
                {
                    if (Props.targetFuelLevelConfigurable)
                    {
                        Command_Celes_SetTargetFuelLevel command_SetTargetFuelLevel = new Command_Celes_SetTargetFuelLevel();
